use utf8;

require "$admcmd/z_thread.pl";

my $opt = $cgi->param('opt');
my $submit = $cgi->param('submit');
$cmd_str = '<input type="hidden" name="cmd" value="res_clr">'."\n";

if ($opt eq 'exe') {
	clr_exe($submit);
} elsif ($opt eq 'select') {
	clr_select($submit);
} else {
	clr_menu($submit);
}

sub clr_menu {
	my $submit = shift;
	if ($submit eq $modoru) {
		show_menu();
	}
	my $board = ($type eq 'kako' ? $bbs.'_kako' :$bbs);
	header("どのスレッドのレスを透明削除しますか？");
	if (out_thread(0,$board)) {
		submit_select();
	} else {
		submit_ret();
	}
	print $cmd_str;
	echo '<input type="hidden" name="opt" value="select">',"\n";
}

sub clr_select {
	my $submit = shift;
	if ($submit eq $modoru) {
		show_menu();
	}
	my $key = $cgi->param('dat_name');
	my $thr;
	if ($key eq '') {
		clr_menu();
		footer();
	}
	my $board = ($type eq 'kako' ? $bbs.'_kako' :$bbs);
	($key,$thr) = split(/\.dat<>/,$key);
	open(LOG,"<../$board/dat/$key.dat") or error_exit($board.$key."ファイルのオープンに失敗しました");
	flock(LOG,1);
	if (!open(IFO,"<../$board/ifo/$key.cgi")) {
		close(LOG);
	 	error_exit('ifoファイルのオープンに失敗しました');
	}
	flock(IFO,1);
	header(dec_str($thr));
	while(<LOG>) {
		my ($name,$mail,$info,$message,$title,$admin) = split( /<>/,$_);
		my $cnt = $.;
		$ifo = <IFO>;
		res_view('透明削除',$cnt,\$name,\$mail,\$info,\$message,\$ifo,$admin);
	}
	close(LOG);
	close(IFO);
	submit_exe('選択したレスを透明削除');
	print $cmd_str;
	echo "<input type='hidden' name='opt' value='exe'>\n";
	echo "<input type='hidden' name='key' value='$key'>\n";
}

sub clr_exe {
	my $submit = shift;
	if ($submit eq $modoru) {
		clr_menu();
		footer();
	}
	my @num =  $cgi->param('num');
	$key = $cgi->param('key');
	my $board = ($type eq 'kako' ? $bbs.'_kako' :$bbs);
	my $tmp1 = time().'.tmp';
	my $tmp2 = "../$board/ifo/$tmp1";
	my $tmp1 = "../$board/dat/$tmp1";
	my $fname1 = "../$board/dat/$key.dat";
	my $fname2 = "../$board/ifo/$key.cgi";
	my $sig = (get_index(1,$board,$key) < 0 ? -1 : 1);
	my $cnt = 0;
	my $del = shift(@num);
	my $smn_path = "../$board/smn/$key";
	my $img_path = "../$board/img/$key";
	my $pic_path = get_top() . "$board/img/$key/";
	my $smn = -d $smn_path;
	my $img = -d $img_path;
	get_setting($bbs);
	get_subject();
	open(DAT,"+<$fname1") or error_exit('レスの削除に失敗しました');
	flock(DAT,2);
	open(DTM,"+>$tmp1") or (close(DAT) and error_exit('レスの削除に失敗しました'));
	unless (open(IFO,"+<$fname2")) {
		close(DAT);
		close(DTM);
		error_exit('レスの削除に失敗しました');
	}
	flock(IFO,2);
	unless (open(ITM,"+>$tmp2")) {
		close(DAT);
		close(DTM);
		close(IFO);
		error_exit('レスの削除に失敗しました');
	}
	seek(DAT,0,0);
	seek(IFO,0,0);
	while(<DAT>) {
		$line1 = $_;
		$line2 = <IFO>;
		if ($. == $del) {
			if ($del == 1) {
				print DTM $line1;
				print ITM $line2;
			} else {
				if ($smn && -e "$smn_path/$del.jpg") {unlink "$smn_path/$del.jpg";}
				if ($img && -e "$img_path/$del.jpg") {unlink "$img_path/$del.jpg";}
				if ($img && -e "$img_path/$del.gif") {unlink "$img_path/$del.gif";}
				$cnt++;
			}
			$del = shift(@num);
		} else {
			if ($cnt) {
				my $from = $.;
				my $to = $from - $cnt;
				if ($smn && -e "$smn_path/$from.jpg") {rename "$smn_path/$from.jpg" , "$smn_path/$to.jpg";}
				if ($img) {
					my $hit = 0;
					if (-e "$img_path/$from.jpg") {
						rename "$img_path/$from.jpg" , "$img_path/$to.jpg";
						$from = "$pic_path$from.jpg";
						$to = "$pic_path$to.jpg";
						$hit = 1;
					} elsif (-e "$img_path/$from.gif") {
						rename "$img_path/$from.gif" , "$img_path/$to.gif";
						$from = "$pic_path$from.gif";
						$to = "$pic_path$to.gif";
						$hit = 1;
					}
					if ($hit) {
						my $point = index($line1,$from);
						if ($point >= 0) {
							substr($line1,$point,length($from),$to);
						} else {
							$point = index($line2,$from);
							if ($point >= 0) {substr($line2,$point,length($from),$to);}
						}
					}
				}
			}
			print DTM $line1;
			print ITM $line2;
		}
	}
	seek(DAT,0,0);
	seek(IFO,0,0);
	seek(ITM,0,0);
	seek(DTM,0,0);
	print DAT <DTM>;
	print IFO <ITM>;
	truncate(IFO,tell(IFO));
	truncate(DAT,tell(DAT));
	close(IFO);
	close(DAT);
	close(DTM);
	close(ITM);
	unlink $tmp1,$tmp2;
	header('レス削除');
	echo "<td>$cnt",'件削除しました<br>';
	if ($cnt > 0) {
		echo makeindex($board,$key,$sig),"<br>\n";
		$cnt = abs(get_index(1,$board,$key));
		if (open(SBJ,"+<../$board/subject.txt")) {
			flock(SBJ,2);
			my @list;
			while(<SBJ>) {
				if (index($_,"$key.dat") == 0) {
					$_ = substr($_,0,rindex($_,'('));
					$_ .= "($cnt)\n";
				}
				push(@list,$_);
			}
			seek(SBJ,0,0);
			foreach $data(@list) {
				print SBJ $data;
			}
			truncate(SBJ,tell(SBJ));
			close(SBJ);
		} else {
			echo "subject.txtの書き換えに失敗しました<br>";
		}
	}
	if ($type ne 'kako') {
		if ($ifo{'img_lib'}) {
			require "$dir/test/sub/smn".$ifo{'img_lib'}.'.pl';
		} else {
			require "$dir/test/sub/smn.pl";
		}
		require "$subcmd/page.pl";
		require "$subcmd/mobile.pl";
		get_setting($bbs);
		get_subject();
		put_pc();
		put_subback();
		put_mobile();
	}
	submit_ret();
}
1;